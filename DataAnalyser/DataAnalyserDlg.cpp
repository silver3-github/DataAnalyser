...
void CDataAnalyserDlg::OnBnClickedDataSearch()
{
	if (searchDlg.DoModal() == IDOK && searchDlg.value.GetLength() > 0)
	{
		...
		if (searchDlg.dataType == "命令")
		{
			...
			{
				CString cmdId;
				cmdId = msgList.GetItemText(i, 1).SpanIncluding("0123456789");
				if (cmdId == value)
				{
		...
		if (searchDlg.dataType == "文本")
		{
			for (int i = startItem; i < msgList.GetItemCount(); i++)
			{
				int ret = msgList.GetItemText(i, 2).Find(searchDlg.value);
				if (ret != -1)
				{
					msgList.SetItemState(i, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
					dataAnalyDlg.dataEdit.SetSel(ret, ret + searchDlg.value.GetLength());
					msgList.SetFocus();
					msgList.EnsureVisible(i, false);
					return;
				}
		...
		for (int i = startItem; i < msgList.GetItemCount(); i++)
		{
			int ret = msgList.GetItemText(i, 2).Find(value);
			if (ret != -1)
			{
				msgList.SetItemState(i, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
				dataAnalyDlg.dataEdit.SetSel(ret, ret + value.GetLength());
				msgList.SetFocus();
				msgList.EnsureVisible(i, false);
				return;
			}
